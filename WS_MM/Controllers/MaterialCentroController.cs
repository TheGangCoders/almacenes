...
namespace WS_MM.Controllers
{
    [CorsOrigin]
    [Authorize]
    public class MaterialCentroController : BaseController
    {
        ...
        public IHttpActionResult PMMM_ListarPorMaterialYCentro(int Material, int Centro)
        {
            try
            {
                DataTable dt = MaterialCentroModelDAL.Instancia.ListarPorEmpresaYMaterialYCentro(JWT.IdEmpresa, Material, Centro);
                return Ok(dt);
            }
            catch (Exception ex)
            {
